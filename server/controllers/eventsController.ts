...
import type {
  EventLog,
  KeyspaceEventNode,
} from "../redis-monitors/models/interfaces";
...
const eventsController: EventsController = {
  ...
  */
    ...
    const getPaginatedEventsData = (
      eventLogSnapshot: EventDetails[]
    ): [number, EventDetails[]] => {
      //Helper function for filtering and indexing the keyspaceSnapshot

      if (keynameFilter) {
        eventLogSnapshot = eventLogSnapshot.filter(
          (eventDetails: EventDetails): boolean => {
    ...
    if (dbIndex >= 0) {
      ...
      const [eventTotal, paginatedData] = getPaginatedEventsData(
        monitor.keyspaces[dbIndex].eventLogSnapshot
      );
      res.locals.events = {
  ...
  */
    ...
    const eventLog: EventLog =
      res.locals.monitors[0].keyspaces[dbIndex].eventLog;
    ...
    const keynameFilter: string = req.query.keynameFilter
      ...
      : "";
    const eventTypeFilters: string[] = req.query.eventTypes
    ...
    while (current) {
      ...
      if (eventTypeFilters.length === 0) {
        ...
        current.key.includes(keynameFilter) &&
